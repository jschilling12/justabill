...
def upgrade() -> None:
    ...
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE votetype AS ENUM ('up', 'down', 'skip');
        EXCEPTION
    ...
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE billstatus AS ENUM ('introduced', 'in_committee', 'passed_house', 'passed_senate', 'in_conference', 'passed_both', 'vetoed', 'enacted');
        EXCEPTION
            WHEN duplicate_object THEN null;
        END $$;
    """)
