...
def _user_bucket_from_counts(up: int, down: int, skip: int, threshold: float = 0.80) -> str:
    total = up + down + skip
    if total <= 0:
        return "skip"
...
def _bill_user_level_counts(db: Session, bill_id: UUID) -> dict[str, int]:
    ...
    - If >=80% are up => user counts as up. If >=80% are down => down. Else => skip (mixed/unsure).
    """
