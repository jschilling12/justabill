...
### 4. n8n Workflows
- **Daily Bill Sync**: Fetches recently updated bills from Congress.gov API and ingests them
...
3. Ensure `.env` contains `CONGRESS_API_KEY` (docker-compose passes this into n8n)
4. Activate workflows (schedule triggers only run when active)

### Workflows
- **Daily Bill Sync** (`daily-bill-sync.json`): Runs daily at 6 AM ET; defaults to a 7-day lookback (override with `SYNC_LOOKBACK_DAYS`) and ingests bills from the latest Congress present in the feed
